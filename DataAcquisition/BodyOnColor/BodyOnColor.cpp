...
HANDLE CBodyOnColor::m_hSerial = CreateFile(L"COM1",
...
float CBodyOnColor::m_PulseLength = 1.0 / 60.0f;
*/

double CBodyOnColor::m_fFreq = 0.0;
...
void CBodyOnColor::ProcessBodyOnColor(INT64 nTime, int nBodyCount, IBody** ppBodies, RGBQUAD* pBuffer, int nWidth, int nHeight)
{
    if (m_hWnd)
    {
						...
						*/
						...
						if (SUCCEEDED(hr))
						{
							for (int j = 0; j < _countof(joints); ++j)
							{
								...
								//Write this joint to it's appropriate array location
								jointBuffer[3*joints[j].JointType+0] = joints[j].Position.X;
							...
							//If file exists, Write jointBuffer to file
							if (m_pDataFile)
							{
								hr = WriteToBinaryFile(jointBuffer, 3*JointType_Count);
							}
		...
		if (m_bRecordData && !m_pDataFile)
		{
			...
			HRESULT hr = OpenBinaryFile(szTrackingDataPath);
			...
			{
				StringCchPrintf(szStatusMessage, _countof(szStatusMessage), L"Failed open binary file to %s", szTrackingDataPath);
			}
		...
		if (!m_bRecordData && m_pDataFile)
		{
			...
			{
				StringCchPrintf(szStatusMessage, _countof(szStatusMessage), L"Failed to stop binary file");
			}
...
D2D1_POINT_2F CBodyOnColor::BodyToScreen(const CameraSpacePoint& bodyPoint)
{
...
/// <summary>
/// Get the name of the file where skeletal tracking will be stored.
/// </summary>
...
/// </returns>
HRESULT CBodyOnColor::GetTrackingFileName(LPWSTR lpszFilePath, UINT nFilePathSize)
{
	...
	if (SUCCEEDED(hr))
	{
		...
		// File name will be KinectTracking-HH-MM-SS.bin
		StringCchPrintfW(lpszFilePath, nFilePathSize, L"%s\\KinectTracking-%s.bin", pszKnownPath, szTimeString);
	}
...
/// <summary>
/// Save skeletal tracking data to a binary file
/// </summary>
/// <param name="TrackingDataPath">Name of binary file</param>
/// <returns>indicates success or failure</returns>
HRESULT CBodyOnColor::OpenBinaryFile(WCHAR *szTrackingDataPath)
{
	
	m_pDataFile = _wfopen(szTrackingDataPath, L"wb");
...
HRESULT CBodyOnColor::WriteToBinaryFile(float *joints, int count)
{
	size_t result;

	result = fwrite(joints, sizeof(float), count, m_pDataFile);
...
HRESULT CBodyOnColor::CloseBinaryFile()
{
	int chk;
...
/// </summary>
void CBodyOnColor::TriggerSerialPort(void * portHandle)
{
